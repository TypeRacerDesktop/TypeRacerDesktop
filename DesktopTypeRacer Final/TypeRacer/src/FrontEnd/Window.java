...
public class Window {
	...
	public Window() {
		...
		Cal30 = new Font(d, "Calibri", 30, SWT.NORMAL);
		Arial53 = new Font(d, "Arial", 53, SWT.NORMAL);
		Arial40 = new Font(d, "Arial", 40, SWT.NORMAL);
		Cal11 = new Font(d, "Calibri", 11, SWT.NORMAL);
		Arial60 = new Font(d, "Arial", 60, SWT.NORMAL);
	}

	public void createCanvas(boolean mainclass) {
	...
	public void startGame(boolean canvas) throws IOException {
	...
	public void rankList(boolean menu) throws IOException {
		escButton();
		...
		if (menu) {
			while (!shell.isDisposed()) {
				if (!d.readAndDispatch()) {
					printUserData(1, true);
					gc.drawText("RankingList: ", wid, 50);
				}
			}
		} else {
			clearScreen();
			levelSelected(1, false);
		}
	}
	...
	public void about(boolean menu) throws IOException {
		gc.setFont(Arial60);
		...
		words = df.drawTheText(con, words, bds.width/19.51);
		...
		username.addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				if (e.character == 27) {
					...
					String newName = username.getText().replaceAll("\\s", "");
					if (newName != "") {
						if (newName.length() > 8) {
	...
	public void levelSelected(final int i, boolean menu) throws IOException {
		...
		String con = df.getContent();
		words = df.drawTheText(con, words, bds.width/8.6);
		parse = df.parseTheText(words);
		...
		if (menu) {
			while (!shell.isDisposed()) {
				if (!d.readAndDispatch()) {
					...
					gc.drawText(
							...
							chWidthHelper - 78, wpmHeightHelper + 100);
					...
					if (speed > 99) {
						...
						gc.drawText(Integer.toString(speed) + " wpm  ",
								chWidthHelper - 78, wpmHeightHelper);
					}
			...
			clearScreen();
			printUserData(1, false);
		}
	}

	public void printUserData(int caseOfCall, boolean game) {
		int rows = 1;
		int cols = 650;
		int cont = 0;
		int counter = 1;
	...
	public void drawAbout(boolean about) throws IOException {
		int rows = width / 6;
		int cols = 0;
		int maxRows = 0;
		gc.setForeground(shell.getDisplay().getSystemColor(SWT.COLOR_BLACK));
